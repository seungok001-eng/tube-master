...
2. 대본 전체를 빠짐없이 분할하세요. 어떤 문장도 생략하지 마세요.
3. 분할 기준 (중요):
   - 기본 원칙: 문장 1개 = 장면 1개
   - 단, 아래 경우에는 인접 문장과 합쳐서 1개 장면으로 만드세요:
     a) 문장이 20자 미만으로 너무 짧을 때 → 의미상 가장 자연스럽게 이어지는 앞 문장이나 뒷 문장과 합치세요
     b) 문장이 불완전하거나 단독으로 의미가 불명확할 때 → 문맥상 연결되는 문장과 합치세요
   - 합친 경우 script는 두 문장을 공백 없이 원문 그대로 이어붙이세요 (줄바꿈 유지)
...
[
  {
    "script": "원문 그대로의 장면 대본 (수정 금지, 병합 시 두 문장 이어붙이기)",
